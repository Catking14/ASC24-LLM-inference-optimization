...
def model_generate(device):
    ...
    llm = LLM(model="/mlsteam/data/data/AquilaChat2-34B-Int4-GPTQ", trust_remote_code=True, dtype="float16", quantization="gptq", tensor_parallel_size=TOTAL_GPUS)
    # llm = LLM(model="/mlsteam/data/data/aquilachat2-34b-int4-gptq-marlin-hf", trust_remote_code=True, dtype="float16", quantization="marlin", tensor_parallel_size=TOTAL_GPUS)
    # llm = LLM(model="/mlsteam/data/data/aquilachat2-34b-int4-gemm-awq-a100", trust_remote_code=True, dtype="float16", quantization="awq", tensor_parallel_size=TOTAL_GPUS)
    ...
    for text in prompts:
        out = llm.generate(text[0], sampling_params)
