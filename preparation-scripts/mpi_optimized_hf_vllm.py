...
def model_generate(device):
    # load dataset
    prompts, max_length, total_data_len = split_workload("ceval/ceval-exam", TOTAL_GPUS)
    # ceval = []
    # prompts = []
    ...
    #     cur = f"{dataset['question'][i]}\n\nA. {dataset['A'][i]}\nB. {dataset['B'][i]}\nC. {dataset['C'][i]}\nD. {dataset['D'][i]}"
    #     prompts.append([cur, dataset["answer"][i]])
    batched_prompts = simple_batch(prompts, 32)
    ...
    # llm = LLM(model="/mlsteam/data/data/AquilaChat2-34B-Int4-GPTQ", trust_remote_code=True, dtype="float16", quantization="gptq")
    # llm = LLM(model="/mlsteam/data/data/aquilachat2-34b-int4-gemm-awq-a100", trust_remote_code=True, dtype="float16", quantization="awq")
    llm = LLM(model="/work/aquilachat2-34b-int4-gptq-marlin-hf", trust_remote_code=True, dtype="float16", quantization="marlin", local_rank=device)
    ...
    if rank == 0:
        ...
        # print(f"Total prediction accuracy is {correct_total / total_data_len * 100} %.")
